...
- (void)checkUnzip:(NSTimer *)timer {
    if ([unzipTask isRunning] == 0) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:[workingPath stringByAppendingPathComponent:kPayloadDirName]]) {
            ...
            if (changeBundleVersionShortCheckbox.state == NSOnState) {
...
- (BOOL)doBundleVersionShortChange:(NSString *)newBundleVersionShort {
    BOOL success = YES;
    
    success &= [self doAppBundleVersionShortChange:newBundleVersionShort];
    success &= [self doITunesMetadataBundleVersionShortChange:newBundleVersionShort];
...
- (BOOL)doAppBundleVersionShortChange:(NSString *)newBundleVersionShort {
    NSArray *dirContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[workingPath stringByAppendingPathComponent:kPayloadDirName] error:nil];
    NSString *infoPlistPath = nil;
...
- (BOOL)changeBundleVersionShortForFile:(NSString *)filePath bundleVersionShortKey:(NSString *)bundleVersionShortKey newBundleVersionShort:(NSString *)newBundleVersionShort plistOutOptions:(NSPropertyListWriteOptions)options {
...
- (IBAction)changeBundleVersionShortPressed:(id)sender {
    ...
    if (sender != changeBundleVersionShortCheckbox) {
        return;
    }
